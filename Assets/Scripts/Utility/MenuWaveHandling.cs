...
namespace Utility
{
    public class MenuWaveHandling : MonoBehaviour
    {
        ...
        private Camera cam;
        private MenuWaveGenerator groundGenerator;

        private void Awake()
        {
            cam = Camera.main;

            groundGenerator = FindAnyObjectByType<MenuWaveGenerator>();
            groundGenerator.OnMapGenerated += WaveFunction_OnMapGenerated;

            midPoint.position = new Vector3(groundGenerator.WaveFunction.GridSize.x * groundGenerator.WaveFunction.GridScale.x / 2.0f,
                groundGenerator.WaveFunction.GridSize.y * groundGenerator.WaveFunction.GridScale.y / 2.0f,
                groundGenerator.WaveFunction.GridSize.z * groundGenerator.WaveFunction.GridScale.z / 2.0f);
        }
